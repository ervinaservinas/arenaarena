...
import {deductGold} from "../features/Character";
...
const Shop = () => {
    ...
    function buyItem(x) {
        if (myCharacter.gold >= x.price && inventory.length < myCharacter.inventorySlots) {
            ...
            console.log("no gold in your inventory");
        }
    ...
    return (
        <div >
            <div className="selectShopCategory d-flex">
                <button   onClick={() => selectCategory("weapons")}>
                    Weapons
                </button>
                <button   onClick={() => selectCategory("potions")}>
                    Potions
                </button>
            </div>
            <div className="shopItems d-flex f-wrap">
                {trader[getSelectedCategory].map((x, i) => <div key={i} className="d-flex itemBox">
                    <img src={x.image} alt=""/>
                    <div>
